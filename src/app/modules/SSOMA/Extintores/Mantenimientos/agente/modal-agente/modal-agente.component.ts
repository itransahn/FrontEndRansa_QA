...
@Component({
  selector: 'app-modal-agente',
  templateUrl: './modal-agente.component.html',
  styleUrls: ['./modal-agente.component.scss']
})
export class ModalAgenteComponent implements OnInit {
  ...
  constructor( 
  private dialogRef:MatDialogRef<ModalAgenteComponent>,
...
cargarFormPost(){
    this.FormModule = new FormGroup({
      TipoAgente    : new FormControl({ value: '', disabled : this.enable }, [Validators.required] )
    })
}
...
SetForm(){
  this.FormModule.setValue({
    TipoAgente     : this.data?.data['tipo_agente']
  })
}
...
insertar(){
    ...
    let params = {
      tipoAgente        : this.FormModule.value.TipoAgente,
    } 
